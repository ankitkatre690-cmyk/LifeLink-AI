...
class ResponderService:
    ...
    def update_status(self, user, status: str):
        ...
        if status in {"Available", "Offline"}:
            active_assignment = self.repository.get_active_assignment_for_responder(profile.id)
    ...
    def create_assignment(self, user, request):
        ...
        active_assignment = self.repository.get_active_assignment_for_emergency(request.emergency_id)
        if active_assignment is not None:
            raise AssignmentAlreadyExists()
        existing = self.repository.get_assignment_for_emergency_and_responder(request.emergency_id, profile.id)
        if existing:
            raise AssignmentAlreadyExists()
        if emergency.status not in {"Pending", "Accepted"}:
            raise ValueError(f"Cannot assign responder to emergency in status: {emergency.status}")
        if profile.status != "Available":
            raise ValueError(f"Responder is not available for assignment: {profile.status}")
    ...
    def update_assignment(self, user, assignment_id: uuid.UUID, status: str, notes: str | None):
        ...
        if status != previous_status:
            try:
                validate_assignment_transition(previous_status, status)
            except ValueError as exc:
                raise ValueError(f"Invalid responder assignment status transition: {previous_status} -> {status}") from exc
